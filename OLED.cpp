...
//const int addr7bit = 0x78;      // 7 bit I2C address
//const int addr8bit = 0x78; // 8bit I2C address, 0x90
/*int main() {
    char cmd[2];
    while (1) {
        cmd[0] = 0x01;
...
/**
  ...
  * @retval 无
  */
const int addr8bit = 0x78;
I2C i2c(D4,D5);//D4 D5
void OLED_I2C_Write(uint8_t address, uint8_t data) 
{
    char cmds[2] = {address, data};
    i2c.start();
    i2c.write(addr8bit, cmds, 2);
    /*i2c.read(0);
    cmds[0]=cmds[1];
    i2c.write(addr8bit, cmds, 1);*/
    i2c.stop();
    /*cmds[0]=cmds[1];
    i2c.write(addr8bit, cmds, 1);*/
}
...
void OLED_WriteData(uint8_t* data,uint8_t Count)
{
    ...
    i2c.start();
    i2c.write(addr8bit, cmds, Count+1);
    i2c.stop();
}
/*void OLED_WriteCommand(uint8_t Command)
{
...
/**
  ...
  */
void OLED_Init(void)
{			//先调用底层的端口初始化	/*写入一系列的命令，对OLED进行初始化配置*/
